...
- **Mode**: defines how the big knob behaves.
   - **Uni**: produces a unipolar signal ranging from 0V to 10V.
   - **Bip**: produces a bipolar signal ranging from -5V to 5V.
...
- **(Output Jack)**: outputs the combined signal: all inputs with their corresponding attenuation plus the main offset defined by the Big Knob.

To use it as a mixer, set the mode to Bip (Bipolar) and control the volumes with the attenuverters.
