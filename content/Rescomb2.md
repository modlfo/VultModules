...
Rescomb 2 can be used as well as a Karplus-Strong like sound generator by using its direct V/OCT input and stimulating it with short pulses.
...
- **Feedback**: controls the level of output signal that is circulated to the input. The feedback can be used to define the decay of the sound when the filter is fed with short pulses. High levels of feedback will make the filter self oscillate.
- **Drive**: controls the level of the input signal. High drive will cause the filter to saturate, producing distortion in the sound. It can be used to add character to the input.
...
- **V/OCT**: this is a direct input to set the frequency of the filter. The input will be offset by the Frequency knob. This input is especially useful when using Rescomb 2 as a sound generator.

### LFO Section

- **Rate**: defines the LFO frequency. It can go from super slow to audio rate.
...
- **Sync (Input)**: receives a trigger signal to reset the LFO to its initial state.
